...
# платы сотрудника. В расчете необходимо использовать формулу: (выработка в часах *
# ставка в час) + премия. Для выполнения расчета для конкретных значений необходимо
# запускать скрипт с параметрами.
from sys import argv

script_name, output_hours, rate_hour, prize = argv

result = (float(output_hours) * float(rate_hour) + float(prize))
print(f'Salary staff: {result}')
#
...
my_list = [21, 1, 2, 17, 123, 1, 44, 44, 3, 2, 10, 7, 4, 11]
print(f'New list ex4: {[el for el in my_list if my_list.count(el) == 1]}')
#
# 5. Реализовать формирование списка, используя функцию range() и возможности генератора.
# В список должны войти четные числа от 100 до 1000 (включая границы). Необходимо получить
# результат вычисления произведения всех элементов списка.
# Подсказка: использовать функцию reduce().
'''Up call function reduce'''


def my_func(el_pr, el):
    return el_pr * el


print(f'Result ex5 {reduce(my_func, [el for el in range(100, 1001) if el % 2 == 0])}')
#
...
'''Up call function count'''


def my_count(start_el, stop_el):
...
my_count(start_el=int(input('Enter start element: ')), stop_el=int(input('Enter stop element: ')))

'''Up call function cycle'''


def my_cycle(var_list, iterator):
    ind = 0
...
my_cycle(var_list=['q', 'a', 'z'], iterator=int(input('Enter iteration: ')))
#
